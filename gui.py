...
class ParkingDetectionGUI:
    def __init__(self):
        ...
        self.camera1 = Camera("NW", "192.168.183.200", 8899, "", "", 19, 25) #18,26
    ...
    def update_video_frame(self, frame, results, label, message_label):
        ...
        for result in results:
            if result['class_name'] == 'helm':
                message_label.configure(text="Silahkan Masuk!", text_color="#ffffff", font=("Poppins", 24, "bold"), fg_color="#457b9d") 
                message_label.pack(padx=10, pady=10)
            if result['class_name'] == 'tanpa-helm':
                message_label.configure(text="Lengkapi Helm!", text_color="#ffffff", font=("Poppins", 24, "bold"), fg_color="#e63946") 
                message_label.pack(padx=10, pady=10)
            if result['class_name'] == 'pejalan':
    ...
    def run(self):
        ...
        self.camera_thread2 = threading.Thread(target=self.camera2.run, args=(self.parking_model4.run, self.parking_model4.run, update_parking,), name="Camera2")
